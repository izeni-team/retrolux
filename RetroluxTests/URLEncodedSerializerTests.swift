...
class URLEncodedSerializerTests: XCTestCase {
    func testSerializer() {
        let request = Builder.dummy().makeRequest(method: .post, endpoint: "test", args: Body<URLEncodedBody>(), response: Void.self)
        ...
        let response = request(Body(body)).test()
    ...
    func testWithoutBodyWrapper() {
        let request = Builder.dummy().makeRequest(method: .post, endpoint: "test", args: URLEncodedBody(), response: Void.self)
        let body = URLEncodedBody(values: [
            ("Hello", "3"),
            ("Another", "Way"),
            ("Test", "Yay!"),
        ...
        let response = request(body).test()
        XCTAssert(response.request.httpBody! == "Hello=3&Another=Way&Test=Yay!&3+3%3D=24/4&Another=&=&Misc=!@%23$%25%5E%26*()%3D:/?%20%22'".data(using: .utf8)!)
        XCTAssert(response.request.value(forHTTPHeaderField: "Content-Type") == "application/x-www-form-urlencoded")
    }
    
    func testFields() {
        let request = Builder.dummy().makeRequest(type: .urlEncoded, method: .post, endpoint: "test", args: (Field("first_name"), Field("last_name")), response: Void.self)
        let response = request((Field("Christopher Bryan"), Field("Henderson"))).test()
